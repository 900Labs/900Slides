...
if grep -rnE '(/Users/[^/]+|/home/[^/]+|[A-Z]:\\)' \
    --include='*.rs' --include='*.ts' --include='*.js' --include='*.svelte' \
...
echo "==> Checking for application network request APIs"
if grep -rnE '\b(fetch|WebSocket|XMLHttpRequest|EventSource)[[:space:]]*\(|sendBeacon[[:space:]]*\(' \
    --include='*.ts' --include='*.js' --include='*.svelte' \
    apps/desktop/src/ 2>/dev/null; then
    echo "FAIL: application network request API found."
    status=1
fi
